...
let ModalVote = React.createClass({
  ...
  add() { //添加投票项目
    ...
    form.validateFields(['voteItemName', 'voteItemPersons', 'upload'], (errors, values)=> {
      ...
      form.setFieldsValue({voteItemName: undefined, voteItemPersons: undefined, upload: undefined});
    });
  },
  handleSubmit(e) {
    e.preventDefault();
    this.props.form.validateFields(['voteStartDate', 'voteEndDate','voteStartTime', 'voteEndTime', 'upload'], (errors, values) => {
      if (!!errors) {
        console.log('Errors in form!!!');
        return;
      }
  ...
  render() {
    ...
    return (
        <div>
          <Modal visible={this.props.visible} ref="modal"
            ...
            <Button key="submit" type="primary" size="large" onClick={this.handleSubmit}>提交</Button>
          ]}>
            <Form horizontal form={this.props.form}>
              ...
              <Row>
                ...
                <Col span={16} offset={6} style={{marginTop:'-60px',marginBottom:'10px'}}>
                  <Table columns={columns} dataSource={this.state.voteItems} pagination={false} size="small"/>
                </Col>
              </Row>
              {this.getAddForm()}
            </Form>
